...
import type { NephrologyMedicationOrder } from '../../types';
import { NephrologyNav } from './NephrologyNav';

export function NephrologyMedicationsPage() {
    ...
    const { data, isLoading } = useQuery({
        queryKey: ['nephrology-medications', page, search, isActive],
        queryFn: () =>
            nephrologyMedicationService.getOrders({
                page,
                limit,
    ...
    const deleteMutation = useMutation({
        mutationFn: (id: string) => nephrologyMedicationService.deleteOrder(id),
        onSuccess: () => {
    ...
    const handleDelete = (order: NephrologyMedicationOrder) => {
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-bold text-gray-900">Nephrology Medications</h1>
                    <p className="text-gray-500 mt-1">Manage kidney-related medication regimens.</p>
                </div>
                <Link to="/nephrology/medications/new">
                    <Button>
                            ...
                            {isLoading ? (
                                ...
                                orders.map((order) => (
                                    <tr key={order.id} className="hover:bg-gray-50">
                                        ...
                                        <td className="px-6 py-4">
                                            <span className={`px-2 py-1 rounded-full text-xs font-semibold ${order.isActive ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-700'}`}>
                                                {order.isActive ? 'Active' : 'Inactive'}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-2">
                                                <Link to={`/nephrology/medications/${order.id}/edit`}>
