...
import type { CardiologyMedicationRoute, CreateCardiologyMedicationOrderRequest } from '../../types';
import type { SelectOption } from '../../components/ui/SearchableSelect';
import { CardiologyNav } from './CardiologyNav';

const routeOptions: CardiologyMedicationRoute[] = ['PO', 'IV', 'IM', 'SC', 'SL', 'OTHER'];
...
export function CardiologyMedicationFormPage() {
    ...
    const { data: order, isLoading } = useQuery({
        ...
        queryFn: () => cardiologyMedicationService.getOrder(id!),
        enabled: isEditMode,
    });
    ...
    const { data: patientsData, isLoading: isPatientsLoading } = useQuery({
        queryKey: ['patients', 'cardiology-medication', patientSearch],
        queryFn: () =>
    ...
    const { data: providersData, isLoading: isProvidersLoading } = useQuery({
        queryKey: ['providers', 'cardiology-medication', providerSearch],
        queryFn: () =>
    ...
    const createMutation = useMutation({
        mutationFn: (payload: CreateCardiologyMedicationOrderRequest) => cardiologyMedicationService.createOrder(payload),
        onSuccess: () => {
    ...
    const updateMutation = useMutation({
        mutationFn: (payload: CreateCardiologyMedicationOrderRequest) => cardiologyMedicationService.updateOrder(id!, payload),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['cardiology-medications'] });
            queryClient.invalidateQueries({ queryKey: ['cardiology-medication', id] });
            navigate('/cardiology/medications');
        },
        onError: (err: Error) => {
            setError(err.message);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const payload: CreateCardiologyMedicationOrderRequest = {
    ...
    if (isEditMode && !order) {
        return (
            <div className="text-center py-12">
                ...
                <Link to="/cardiology/medications" className="text-rose-600 hover:underline mt-2 inline-block">
                    Back to medications
                </Link>
            </div>
        );
    ...
    return (
        <div className="w-full max-w-5xl mx-auto space-y-6 lg:pl-0">
            <div className="flex items-center gap-4 pt-2 lg:pt-0">
                <Link to="/cardiology/medications">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    onChange={(e) => setFormData((prev) => ({ ...prev, route: e.target.value as CardiologyMedicationRoute }))}
                                >
